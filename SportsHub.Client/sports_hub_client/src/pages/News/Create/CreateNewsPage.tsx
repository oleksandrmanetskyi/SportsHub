...
import NewssApi, { createNews, editNews, getNews } from '../../../api/newsApi';
...
export default function CreateNewsPage () {
    ...
    const onFinish = (values: any) => {
        ...
        news.title = form.getFieldValue("name");
        news.description = form.getFieldValue("description");
        news.imagePath = fileName;

        let sportId = form.getFieldValue("sportKind");
        if(typeof sportId == 'string')
        {
          news.sportKindId = sportKinds?.find(x => x.name = sportId)?.id ?? 0;
        }
        else 
        {
          news.sportKindId = sportId;
        }
        ...
        {
          news.id = +id;
            editNews(JSON.stringify(news)).then(() => {
                notificationLogic("success", "News is edited succesfully");
                history.push(`/News/get/${id}`);
              })
              .catch(() => {
                notificationLogic("error", "Failed to edite News");
              });;
        }else
        {
        createNews(JSON.stringify(news)).then(() => {
            notificationLogic("success", "News is created succesfully");
            history.push(`/News`);
          })
          .catch(() => {
            notificationLogic("error", "Failed to create News");
          });;
        }
    ...
    const fetchData = async () => {
        ...
        // await NewssApi.getAll().then(r => setNewss(r));
    ...
    //   }}
    >
        {id ? <h2 className="profileHeader">Edit {news.title} News</h2> : <h2 className="profileHeader">Create News</h2>}
      ...
      <Form.Item
        ...
        label="Kind of sport"
        hasFeedback
        initialValue={sportKinds?.find(x => x.id == news.sportKindId)?.name}
        rules={[{ required: true, message: 'Please select kind of sport for this news' }]}
      >
